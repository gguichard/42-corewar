...
/*   Updated: 2019/02/14 03:36:18 by wta              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_error	is_skippable(int fd, char **line)
{
	int	ret;
...
t_error	get_name(t_data *data, int fd, char **line)
{
	...
	err_id = ERR_NOERROR;
	str = NULL;
	if ((ft_strnequ(*line, ".name", 5) == 0)
			|| ((str = get_dquote((*line) + 5)) == NULL))
		err_id = ERR_BADFMT;
...
void	set_op_tab(t_op op_tab[])
{
	...
	op_tab[5] = (t_op){"sub", 3, {T_REG, T_REG, T_REG}, 5, 10, "soustraction"
		, 1, 0};
	op_tab[6] = (t_op){"and", 3, {T_REG | T_DIR | T_IND, T_REG | T_IND | T_DIR
		, T_REG}, 6, 6, "et (and  r1, r2, r3   r1&r2 -> r3", 1, 0};
	op_tab[7] = (t_op){"or", 3, {T_REG | T_IND | T_DIR, T_REG | T_IND | T_DIR
		, T_REG}, 7, 6, "ou  (or   r1, r2, r3   r1 | r2 -> r3", 1, 0};
	op_tab[8] = (t_op){"xor", 3, {T_REG | T_IND | T_DIR, T_REG | T_IND | T_DIR
...
t_error	read_file(char *file, t_data *data)
{
	...
	if (err_id == ERR_NOERROR)
	{
		if ((err_id = is_skippable(fd, &line)) != ERR_NOERROR)
			return (err_id);
